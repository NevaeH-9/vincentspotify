...
```
a box plot shows the valence (musical positivity) of Pitbull's songs over time, positive in general but become extremely neagtive around 2018
...
rds |>
  ...
  theme(legend.position = "bottom")
```
scatter plot of relationships between valence and energy, showing a positive correlation when Valence < 0.5.
```{r}
#| label: fig-4
...
rds |>
  ...
  theme(legend.position = "bottom")
```
scatter plot of relationships between Energy and Danceability, showing a positive correlation when Energy < 0.5.
